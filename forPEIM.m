...
% ATTENTION!!!!!!!!!
% ROI can not be sticked on the edge. If not, MATLAB cannot find the ROI
% LOCATION.
...
%%
Frequency = zeros(size(expTab, 1), 1);
MoS2 = zeros(size(expTab, 1), 1);
sur = zeros(size(expTab, 1), 1);
bg = zeros(size(expTab, 1), 1);
for ii = 1:size(expTab, 1)
    Frequency(ii, 1) = expTab(ii).Frequency(1, 1);
    MoS2(ii, 1) = expTab(ii).ampl(1, 1);
    sur(ii, 1) = expTab(ii).ampl(2, 1);
    bg(ii, 1) = expTab(ii).ampl(3, 1);
end


%%
Y = bg;
c1 = polyfit(Frequency, Y, 1); d1 = polyval(c1, Frequency);

figure('color', 'w');
plot(Frequency, Y, '.', 'MarkerSize', 12);
hold on
plot(Frequency, d1, '--', 'color', [0.8500, 0.3250, 0.0980], 'LineWidth', 2);
xlabel('Frequency (Hz)')
ylabel('Capacitance (a.u.)')
hold off

%%
prefix = ('G:\EDL\MoS2_1011_bareAu\_Result\mat\');
d = sortObj(dir([prefix, '*.mat']));
for ii = 1:size(expTab, 1)
    load([prefix, d(ii).name]);

    expTab(ii).ROImean = Value.A_peak;
...
%%
X = (1:1024)';
curve = lowp(A, 13, 38, 0.1, 20, 100); % A2;
plot(X, A,  X, curve)
xlim([200 400])
