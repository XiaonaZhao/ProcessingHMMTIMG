...
tifFile = 'G:\MoS2\MoS2_0802\B3_Ru_-0-1 -0-3V_100mV_2c_HMMT100fps';
[Value(1).tifPath, Value(1).tifName] = ReadTifFileNames(tifFile);

tifFile = 'G:\MoS2\MoS2_0802\B4_PBS_-0-1 -0-3V_100mV_2c_HMMT100fps';
[Value(2).tifPath, Value(2).tifName] = ReadTifFileNames(tifFile);

tifFile = 'G:\MoS2\MoS2_0802\C2_bg_Ru_-0-1 -0-3V_100mV_2c_HMMT100fps';
...
Value(1).sROI = sROI{1, 1};
Value(2).sROI = sROI{1, 2};
% Value(3).sROI = sROI{1, 9};
...
[row2, col2] = ImageJroiLocation(Value(1).sROI);
...
%%
tic
TifSaveRoute = 'G:\MoS2\MoS2_0802\_Result_f\TIF_B3_B4';
...
current_line = zeros(length(Current), length(sROI));
...
%%
f = fit(X_line, f_line,'gauss1');
figure('color','white');
plot(f,X_line, f_line)
...
fX = (1:L)';
figure('color', 'white');
filterCurve = lowp(Curve, 15*0.1, 15, 0.01, 20, 100);
plot(fX, Curve, fX, filterCurve)
%%
ii = 523;
...
%%
savepath = 'G:\MoS2\MoS2_0802\_Result_f\TIF_B3_B4_concentration\';
for ii = 400:800
%     ii = 524;
    tif_ii = exp{ii, 1} - exp{399, 1};
